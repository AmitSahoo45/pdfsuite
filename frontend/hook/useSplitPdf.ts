...
export function useSplitPdf({ pdfFiles, setIsLoading }: UseSplitPdfOptions) {
    ...
    const handleSplitPdf = useCallback(async () => {
        ...
        try {
            ...
            const modeLabel = getSplitModeLabel(splitMode);
            if (splitResult.outputs.length === 1) {
                ...
                const zipEntries = splitResult.outputs.map((output) => ({
                    filename: output.filename,
                    data: output.bytes,
                }));
                ...
                if (estimatedZipSize >= ZIP_MEMORY_WARNING_THRESHOLD_BYTES) {
                    toast(
                        'Large ZIP detected. Mobile devices may need more memory. Downloading the ZIP is still available.'
                    );
                }
